package com.trading.engine.service.mock;

import lombok.extern.slf4j.Slf4j;
import org.springframework.boot.autoconfigure.condition.ConditionalOnProperty;
import org.springframework.context.annotation.Primary;
import org.springframework.stereotype.Service;

import java.math.BigDecimal;
import java.math.RoundingMode;
import java.util.Random;

@Service
@Primary
@ConditionalOnProperty(name = "mock.enabled", havingValue = "true")
@Slf4j
public class MockMarketDataService {

    private final Random random = new Random();

    public BigDecimal getCurrentPrice(final String symbol) {
        log.debug("[MOCK] Getting current price for {}", symbol);

        final var basePrice = switch (symbol) {
            case "BTCUSDT" -> new BigDecimal("43000");
            case "ETHUSDT" -> new BigDecimal("2200");
            case "BNBUSDT" -> new BigDecimal("300");
            default -> new BigDecimal("100");
        };

        final var variation = (random.nextDouble() - 0.5) * 0.04;
        return basePrice.multiply(BigDecimal.ONE.add(BigDecimal.valueOf(variation)))
                .setScale(2, RoundingMode.HALF_UP);
    }

    public Double getOpenInterestChangePercent(final String symbol) {
        log.debug("[MOCK] Getting OI change for {}", symbol);
        return -5.0 + (random.nextDouble() * 15.0);
    }

    public Double getFundingRate(final String symbol) {
        log.debug("[MOCK] Getting funding rate for {}", symbol);
        return (random.nextDouble() - 0.5) * 0.002;
    }

    public BigDecimal getATR(final String symbol, final int periods) {
        log.debug("[MOCK] Calculating ATR for {} with {} periods", symbol, periods);

        final var price = getCurrentPrice(symbol);
        final var atrPercent = 0.01 + (random.nextDouble() * 0.01);

        return price.multiply(BigDecimal.valueOf(atrPercent))
                .setScale(8, RoundingMode.HALF_UP);
    }

    public String getVolatilityState(final String symbol) {
        log.debug("[MOCK] Getting volatility state for {}", symbol);

        final var rand = random.nextInt(3);
        return switch (rand) {
            case 0 -> "expanding";
            case 1 -> "contracting";
            default -> "stable";
        };
    }

    public BigDecimal get24hHigh(final String symbol) {
        log.debug("[MOCK] Getting 24h high for {}", symbol);
        final var current = getCurrentPrice(symbol);
        return current.multiply(new BigDecimal("1.05"));
    }

    public BigDecimal get24hLow(final String symbol) {
        log.debug("[MOCK] Getting 24h low for {}", symbol);
        final var current = getCurrentPrice(symbol);
        return current.multiply(new BigDecimal("0.95"));
    }
}
